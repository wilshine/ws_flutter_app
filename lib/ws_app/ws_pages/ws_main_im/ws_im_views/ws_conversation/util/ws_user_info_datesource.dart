...
class UserInfoDataSource {
  static Map<String, WSUserModel> cachedUserMap = {}; //保证同一 userId
  static Map<String, GroupInfo> cachedGroupMap = {}; //保证同一 groupId
  static UserInfoCacheListener? cacheListener;

  // 用来刷新用户信息，当有用户信息更新的时候
  static void setUserInfo(WSUserModel? info) {
    if (info == null) {
      return;
    }
    cachedUserMap[info.userId] = info;
    // WSDbManager.instance.setUserInfo(info);
  }

  // 获取用户信息
  static Future<WSUserModel> getUserInfo(String userId) async {
    WSUserModel? cachedUserInfo = cachedUserMap[userId];
  ...
  static void setGroupInfo(GroupInfo info) {
    ...
    WSDbManager.instance.setGroupInfo(info);
  }
  ...
  static Future<GroupInfo> getGroupInfo(String groupId) async {
    ...
    if (cachedGroupInfo != null) {
      ...
      await WSDbManager.instance.getGroupInfo(groupId: groupId);
